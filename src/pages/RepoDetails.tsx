...
import { useGithubRepoDetails, useGithubCommits } from '@/hooks/useGithub';
import { 
  Paper, Typography, CircularProgress, Box, Chip, Divider, Avatar 
} from '@mui/material';
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto p-4 max-w-4xl">
        ...
        <Link to={`/profile/${username}`} className="flex items-center gap-2 text-blue-600 mb-4 hover:underline">
                ...
                <div className="p-4 hover:bg-gray-50 transition flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex flex-col items-start gap-2 mt-1 text-sm text-gray-500 md:flex-row">
                      <span className="font-bold">{item.commit.author.name}</span>
                      <span>comitou em {new Date(item.commit.author.date).toLocaleDateString('pt-BR')}</span>
                    </div>
                  </div>
                </div>
